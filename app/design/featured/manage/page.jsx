...
export default function FeaturedDesignsManagePage() {
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex items-center gap-4">
            <Link href="/design/featured" className="text-sm text-gray-700 hover:underline font-medium">
              View featured page
            </Link>
            <Link href="/design" className="text-sm text-lv-red hover:underline font-medium">
          ...
          {loadingFeatured ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              {featuredDesigns.map((d) => (
                <div key={d.id} className="border border-yellow-200 bg-yellow-50 rounded-lg overflow-hidden flex flex-col">
                  <div className="relative aspect-square bg-gray-100">
                    {d.main_preview_url ? (
                      <Image src={d.main_preview_url} alt={d.title} fill className="object-cover" sizes="(max-width: 640px) 100vw, 25vw" />
                    ) : (
        ...
        {/* All Designs */}
        <section>
          <h2 className="text-lg font-semibold text-gray-800 mb-4">All Designs</h2>
          <div className="mb-4">
            <input
              type="text"
                    ...
                    <tr key={d.id} className="hover:bg-gray-50 transition-colors">
                      <td className="px-4 py-3">
                        <div className="relative w-12 h-12 rounded bg-gray-100 overflow-hidden shrink-0">
                          {d.main_preview_url ? (
                            <Image src={d.main_preview_url} alt={d.title} fill className="object-cover" sizes="48px" />
                          ) : (
                            <div className="w-full h-full flex items-center justify-center text-gray-300 text-[10px]">—</div>
                          )}
                        </div>
                      </td>
                      <td className="px-4 py-3 font-medium text-gray-800 max-w-[200px] truncate">{d.title}</td>
                      <td className="px-4 py-3 text-gray-500 hidden sm:table-cell">{d.main_category_name || "—"}</td>
                      <td className="px-4 py-3 hidden md:table-cell">
                        <span
                          className={`inline-block px-2 py-0.5 rounded text-xs font-medium ${
                            d.status === "published" ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"
                          }`}
                        >
                          {d.status}
                        </span>
