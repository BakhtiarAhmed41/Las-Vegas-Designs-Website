    ...
    <section className="relative min-h-[82vh] overflow-hidden bg-white flex items-center justify-center px-4 sm:px-6 lg:px-8">
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-0 lg:gap-12 items-center text-center lg:text-left">
          ...
          >
            <span className="inline-flex items-center justify-center lg:justify-start rounded-full bg-lv-red/10 text-lv-red text-[11px] font-semibold tracking-wide uppercase px-3 py-1 w-fit mx-auto lg:mx-0">
              Custom production files
            </span>

            <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold leading-tight text-lv-blue">
              {slide.title}
            </h1>

            <p className="text-2xl md:text-3xl font-bold text-lv-red">
              Starting $10 USD
            </p>

            <p className="text-slate-600 text-lg md:text-xl pb-2">
              2 to 6 Hours Urgent Turnaround Time
            </p>

            <a
              className={`${styles.quoteBtn} group relative inline-flex items-center justify-center px-8 py-4 bg-lv-red hover:bg-lv-red-dark text-white text-base md:text-lg rounded-full shadow-md transition-colors`}
          ...
          <div className="order-1 lg:order-2 relative flex items-center justify-center">
            ...
            >
              <div
                className={`absolute transform rotate-1 ${styles.polaroidContainer}`}
                style={{ zIndex: 40 }}
                role="img"
              ...
              >
                <div className={`${styles.floatWrap} ${styles.floatMain}`}>
                  <Image
                    ...
                    height={1200}
                    priority
                    className={`object-cover w-96 md:w-96 lg:w-[520px] ${styles.polaroidImage} shadow-2xl`}
                  />
                </div>
              </div>
            </div>
          ...
          {slides.map((_, idx) => (
            <button
              ...
              className={`transition-all duration-300 rounded-full ${
                idx === currentSlide
                  ? "w-12 h-3 bg-linear-to-r from-lv-sky-blue to-lv-blue"
